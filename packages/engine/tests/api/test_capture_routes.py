...
def test_ingest_calls_import_scan_with_the_requests_own_values(client, monkeypatch):
    seen = []
    monkeypatch.setattr(legacy, "import_scan", lambda conn, path, paper, child, actor, pages=None,
                        masks=None, narrative=False: (seen.append((path, paper, child, actor, pages, masks, narrative)) or
                        {"capture_id": "cap-1", "pages": 2, "results": [1, 2], "unmatched": [], "notes": []}))
    r = client.post("/ingest", headers=HEADERS, json={
        "path": "/scans/a.pdf", "paper_code": "P1", "child_id": "c1", "actor": "aseem",
        "pages": [1, 2], "masks": {"1": 0.2}, "narrative": True,
    })
    assert r.status_code == 200
    body = r.json()
    assert body == {"capture_id": "cap-1", "pages": 2, "n_results": 2, "n_unmatched": 0, "n_notes": 0, "already": False}
...
def test_ingest_with_the_same_idempotency_key_does_not_call_import_scan_again(client, monkeypatch):
    calls = []
    monkeypatch.setattr(legacy, "import_scan", lambda conn, *a, **k: (calls.append(1) or
                        {"capture_id": "cap-2", "pages": 1, "results": [], "unmatched": [], "notes": []}))
...
def test_ingest_a_call_already_in_flight_is_a_409_not_an_unhandled_500(client, conn, monkeypatch):
    ...
    conn.execute(
        ...
        " values (%s,'ingest','http','stuck','running')", (tenant_id,))
    monkeypatch.setattr(legacy, "import_scan", lambda *a, **k: pytest.fail("must not be called"))

    r = client.post("/ingest", headers={**HEADERS, "Idempotency-Key": "stuck"}, json={
        "path": "/scans/c.pdf", "paper_code": "P1", "child_id": "c1", "actor": "aseem",
    })
...
def test_commit_a_different_child_with_the_same_explicit_key_still_only_runs_once_per_key(client, monkeypatch):
