...
def test_fill_calls_bank_fill_with_the_requests_own_values(client, monkeypatch):
    seen = []
    monkeypatch.setattr(bank, "fill", lambda conn, code, difficulty, n: (
        seen.append((code, difficulty, n)) or
        ({"asked": 5, "accepted": 5}, {}, [Item("k1"), Item("k2")])))
    r = client.post("/bank/fill", headers=HEADERS, json={"skill_set": "ADD.2D.REG", "difficulty": "Hard", "count": 5})
...
def test_a_retried_call_with_the_same_key_does_not_ask_the_model_again(client, monkeypatch):
    calls = []
    monkeypatch.setattr(bank, "fill", lambda conn, code, difficulty, n: (
        calls.append(1) or ({"accepted": len(calls)}, {}, [])))
...
def test_an_excessive_count_is_rejected_before_touching_the_model(client, monkeypatch):
    ...
    monkeypatch.setattr(bank, "fill", lambda *a, **k: pytest.fail("must not be called"))
    r = client.post("/bank/fill", headers=HEADERS, json={"skill_set": "X", "difficulty": "Hard", "count": 10_000})
    assert r.status_code == 422
