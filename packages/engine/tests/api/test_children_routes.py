...
def test_find_returns_the_child_id(client, monkeypatch):
    ...
    r = client.post("/children/find", headers=HEADERS, json={"section": "G2", "first_name": "Advika", "actor": "n8n"})
...
def test_two_identical_lookups_both_run_and_both_log(client, monkeypatch):
    ...
    client.post("/children/find", headers=HEADERS, json={"section": "G2", "first_name": "Advika", "actor": "n8n"})
    client.post("/children/find", headers=HEADERS, json={"section": "G2", "first_name": "Advika", "actor": "n8n"})
    assert len(calls) == 2
