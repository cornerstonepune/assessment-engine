...
def test_rebuild_with_no_child_id_rebuilds_everyone(client, monkeypatch):
    seen = []
    monkeypatch.setattr(
        graph_route.skill_graph, "rebuild", lambda conn, child_id: seen.append(child_id) or 26
    )
    ...
    r = client.post(
        "/graph/rebuild", headers={**HEADERS, "Idempotency-Key": "test-rebuild-everyone"}, json={}
    )
...
def test_two_calls_with_the_same_body_and_no_explicit_key_dedupe(client, monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(
        graph_route.skill_graph, "rebuild", lambda conn, child_id: calls.append(1) or len(calls)
    )
...
def test_a_different_body_with_no_explicit_key_runs_independently(client, monkeypatch):
    calls = []
    monkeypatch.setattr(
        graph_route.skill_graph, "rebuild", lambda conn, child_id: calls.append(child_id) or 1
    )
