...
CATALOG = [
    ...
    {"code": "SUB.3D.ZERO", "rung": "R10", "order": 10, "own": "NUM.OPS.02", "skills": {"NUM.OPS.02"}},
...
def test_a_skill_the_child_lags_in_becomes_an_area_to_work_on():
    ...
    assert [a.skill_set for a in areas(graph, CATALOG, RULE)] == ["ADD.2D.REG"]
...
def test_the_weakest_areas_come_first_at_a_level_set_by_how_often_the_child_was_right():
    ...
    assert got[0] == Area("ADD.3D.REG", "NUM.OPS.01", "Medium", 6, 10, "M_NOCARRY", "patterned_error")
...
def test_a_weak_subtraction_on_an_addition_rung_is_worked_on_as_subtraction():
    ...
    assert skill_set_for("NUM.OPS.02", "R9", CATALOG, RULE["reach"]) == "SUB.3D.ZERO"
    assert skill_set_for("NUM.OPS.01", "R9", CATALOG, RULE["reach"]) == "ADD.3D.REG"
...
def test_the_same_area_read_twice_is_worked_on_once_at_its_weakest():
    ...
    got = areas(graph, CATALOG, RULE)
    assert [(a.skill_set, a.state, a.level) for a in got] == [("SUB.3D.ZERO", "emerging", "Easy")]
