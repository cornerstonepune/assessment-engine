...
the very first container start: every route imports `engine.core.db` transitively, so an IndexError
here took down the whole process before a single request could be served."""

from pathlib import Path

from engine.core.db import _repo_root_for


def test_the_monorepo_layout_finds_the_real_repo_root():
    # .../assessment-engine/packages/engine/engine/core/db.py -> .../assessment-engine
    fake = Path("/Users/x/cornerstone/assessment-engine/packages/engine/engine/core/db.py")
...
def test_the_containers_shallow_layout_does_not_raise():
    # Dockerfile: COPY engine ./engine -> /app/engine/core/db.py, nothing five levels up
    fake = Path("/app/engine/core/db.py")
