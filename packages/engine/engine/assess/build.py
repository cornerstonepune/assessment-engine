...
from .ladder import RUNGS, LEVELS
...
def build(outroot="out", grades=("G1","G2","G3","G4"), variants=2, week="W1", week_label="Week __"):
    ...
    with sync_playwright() as pw:
        for g in grades:
            gdir = outroot / g; gdir.mkdir()
            sheets = []
            for lvl in ("Lm", "L0", "Lp"):
                for v in range(1, variants + 1):
                    s = assemble(g, lvl, v, week)
                    k = render_sheet(s, gdir, week_label, pw)
                    sheets.append(dict(sheet_id=k["sheet_id"], level=lvl, variant=v, pages=k["pages"], n_responses=k["n_responses"],
                                       n_items=len(k["items"]), rungs=sorted({i["rung"] for i in k["items"]}),
                                       pdf=f"{g}/{k['sheet_id']}.pdf", key=f"{g}/{k['sheet_id']}.key.json"))
