...
def _cells(sheet_id, item_id, r, big=False, cls=""):
    n = max(1, r.cells)
    s = "".join(f'<span class="cell {cls}" data-s="{sheet_id}" data-i="{item_id}" data-r="{r.rid}" data-k="{k}"></span>' for k in range(n))
...
def _ticks(sheet_id, item_id, r, labels=None):
    ...
    for j, o in enumerate(r.options):
        ...
        out.append(f'<span class="tickopt"><span class="tick" data-s="{sheet_id}" data-i="{item_id}" data-r="{r.rid}" data-k="{j}" data-opt="{html.escape(o)}"></span>{html.escape(lab)}</span>')
...
def _grid(sheet_id, item_id, rows, op, ans_resp, carry=True):
    ...
    for idx, n in enumerate(rows):
        ...
        out.append(f'<div class="g op">{opch if idx == len(rows)-1 else ""}</div>' + "".join(f'<div class="g">{c.strip() or ""}</div>' for c in s))
    out.append('<div class="g blank"></div>' + "".join(f'<div class="g ans cell" data-s="{sheet_id}" data-i="{item_id}" data-r="{ans_resp.rid}" data-k="{k}"></div>' for k in range(w)))
    out.append('</div>')
...
def render_item(sheet, it, n):
    ...
    if f == "bare_sum":
        ...
        body = f'<span class="lab">&#9633; =</span>' + _cells(sid, iid, R["ans"], big) + _work(it.working_lines)
...
<div class="row" style="margin-top:2mm"><span class="lab">exact: {sp["a"]} {_op(sp["op"])} {sp["b"]} =</span>{_cells(sid,iid,R["ans"])}</div>''' + _work(2)
        ...
        body = f'''<div class="grid" style="grid-template-columns: 8.4mm repeat({w}, 8.4mm)">{rowhtml(a,"da")}{rowhtml(b,"db",_op(op))}{rowhtml(c,"none").replace('class="g"','class="g res"')}</div>''' + _work(2)
    elif f == "digit_cards":
        cards = "".join(f'<div class="card">{c}</div>' for c in sp["cards"])
        body = f'<div class="cards">{cards}</div><div class="row"><span class="lab">largest total =</span>{_cells(sid,iid,R["largest"])}</div>' + _text(sid, iid, R["how"], 13)
    elif f == "efficient_method":
        body = f'<div class="row"><span class="lab">answer =</span>{_cells(sid,iid,R["ans"])}</div>' + _text(sid, iid, R["method"], 13)
...
<div class="row"><span class="eq">{a} &minus; {b} =</span>{_cells(sid,iid,R["ans"])}</div>'''
        ...
        body = _ticks(sid, iid, R["tick"], {"yes": "Yes, correct", "no": "No, not correct"}) + _text(sid, iid, R["why"], 15)
    elif f == "find_mistake":
        body = f'<div class="row"><span class="lab">The mistake is in the</span>{_ticks(sid, iid, R["where"])}</div><div class="row" style="margin-top:2mm"><span class="lab">The correct answer is</span>{_cells(sid,iid,R["ans"])}</div>' + _text(sid, iid, R["why"], 14)
    elif f in ("word_1step", "word_2step"):
        body = _work(it.working_lines) + f'<div class="row" style="margin-top:2mm"><span class="lab">Answer</span>{_cells(sid,iid,R["ans"], big)}</div>'
    else:
        body = "".join(_cells(sid, iid, r) for r in it.responses if r.kind == "digits")
    compact = " compact" if f in ("column_grid", "bare_sum", "missing_number", "estimate_then_calc") and sheet.grade != "G1" or (f in ("bare_sum","missing_number") and sheet.grade == "G1") else ""
...
def sheet_html(sheet, week_label="Week __"):
    ...
    buf = io.BytesIO(); qr.save(buf, kind="svg", scale=4, border=1); svg = base64.b64encode(buf.getvalue()).decode()
    ...
    def flush():
        nonlocal cur
        if cur:
            groups.append('<div class="rowgroup">' + "".join(cur) + '</div>' if len(cur) > 1 else cur[0]); cur = []
    for r_html in rendered:
        if 'class="item compact"' in r_html:
            cur.append(r_html)
            if len(cur) == per_row: flush()
        else:
            flush(); groups.append(r_html)
    flush()
    items = "\n".join(groups)
...
def render_sheet(sheet, outdir, week_label="Week __", pw=None):
    ...
    page.pdf(path=str(outdir / f"{sheet.sheet_id}.pdf"), format="A4", print_background=True, margin={"top": "0", "bottom": "0", "left": "0", "right": "0"}, prefer_css_page_size=True)
    browser.close()
    if own:
        pw.stop()
    key = dict(sheet_id=sheet.sheet_id, grade=sheet.grade, level=sheet.level, variant=sheet.variant, week=sheet.week,
               pages=geom["pages"], n_responses=sheet.n_responses(),
               items=[dict(item_id=i.item_id, n=n + 1, template=i.template, rung=i.rung, skills=i.skills, signal=i.signal, fmt=i.fmt,
                           scaffolded=i.scaffolded, stem=i.stem, spec={k: v for k, v in i.spec.items()},
                           responses=[dict(rid=r.rid, kind=r.kind, answer=r.answer, cells=r.cells, options=r.options,
                                           misconceptions=r.misconceptions, tolerance=r.tolerance, rubric=r.rubric, label=r.label) for r in i.responses])
                      for n, i in enumerate(sheet.items)],
               geometry=geom["cells"])
    (outdir / f"{sheet.sheet_id}.key.json").write_text(json.dumps(key, indent=1, ensure_ascii=False), encoding="utf-8")
    return key
