...
class TesseractReader(Reader):
    ...
    def read_digit(self, crop):
        ...
        for psm in ("10", "13", "8"):
            r = subprocess.run(
                ["tesseract", str(tmp), "stdout", "--psm", psm, "-c", "tessedit_char_whitelist=0123456789"],
                capture_output=True,
                text=True,
            )
            s = r.stdout.strip()
...
def mark_sheet(canon_pages, key, reader):
    ...
    for (iid, rid), cells in by_resp.items():
        it = items[iid]
        r = next(x for x in it["responses"] if x["rid"] == rid)
        cells.sort(key=lambda c: c[0]["k"])
        if r["kind"] == "digits":
            chars, confs = [], []
            for g, crop in cells:
                ch, cf = reader.read_digit(crop)
                chars.append(ch)
                confs.append(cf)
            ...
            results.append(
                dict(
                    ...
                    status=status,
                    misconceptions=tags or (["unclassified"] if status == "wrong" else []),
                )
            )
        elif r["kind"] == "tick":
            ...
            results.append(
                dict(
                    ...
                    read=None,
                    confidence=0,
                    status="blank" if ink < 0.005 else "needs_teacher",
                    misconceptions=[],
                    rubric=r.get("rubric"),
                )
            )
...
def simulate_photo(page_png, rng, out_path):
    ...
    noise = rng.integers(-12, 12, desk.shape, dtype=np.int16)
    ...
    def j(s):
        return rng.integers(-int(s), int(s))

    dst = np.array(
        [
            [ox + j(w * 0.06), oy + j(h * 0.05)],
            [ox + w + j(w * 0.06), oy + j(h * 0.05)],
            [ox + w + j(w * 0.06), oy + h + j(h * 0.05)],
            [ox + j(w * 0.06), oy + h + j(h * 0.05)],
        ],
        dtype=np.float32,
    )
    ...
    for (iid, rid), cells in resp_cells.items():
        ...
        if r["kind"] == "digits":
            ...
            if r["misconceptions"] and rng.random() < error_rate:
                chosen = rng.choice(list(r["misconceptions"].keys()))
                val = str(r["misconceptions"][chosen])
            if rng.random() < 0.06:
                val = ""
                chosen = "blank"
            ...
            for g, ch in zip(cells, s):
                if ch.strip():
                    ...
                    cv2.putText(
                        img,
                        ch,
                        (int(cx - 0.32 * g["w"] * ppm), int(cy + 0.33 * g["h"] * ppm)),
            ...
            for g in cells:
                if g["opt"] == pick:
                    x0, y0 = int((g["x"] + 1) * ppm), int((g["y"] + 1) * ppm)
                    x1, y1 = int((g["x"] + g["w"] - 1) * ppm), int((g["y"] + g["h"] - 1) * ppm)
                    cv2.line(img, (x0, y1), (x1, y0), (20, 20, 40), 2)
                    cv2.line(img, (x0, y0), (x1, y1), (20, 20, 40), 2)
...
def roundtrip(sheet_dir, sheet_id, seed=1, workdir="out/_roundtrip"):
    ...
    sheet_dir, workdir = Path(sheet_dir), Path(workdir)
    workdir.mkdir(parents=True, exist_ok=True)
    key = json.loads((sheet_dir / f"{sheet_id}.key.json").read_text())
    rng = np.random.default_rng(seed)
    prng = random.Random(seed)
    subprocess.run(
        ["pdftoppm", "-r", "200", "-png", str(sheet_dir / f"{sheet_id}.pdf"), str(workdir / sheet_id)],
        check=True,
    )
    ...
    for p, canon in canon_pages.items():
        for g, crop in crop_cells(canon, key, p):
            ...
            ncells = sum(
                1
                for x in key["geometry"]
                if x["item"] == g["item"] and x["resp"] == g["resp"] and x["kind"] == "digit"
            )
        ...
        if r["kind"] == "digits":
            expected = (
                ...
                if written == ""
                    ...
                    "correct"
                    if int(written) == int(r["answer"])
                    or (r.get("tolerance") and abs(int(written) - int(r["answer"])) <= r["tolerance"])
                    else "wrong"
                )
            )
            ...
            if chosen and chosen != "blank":
                diag_total += 1
                diag_hits += chosen in r["misconceptions"]
        else:
    ...
    return dict(
        ...
        pages=len(pages),
        qr_decoded=qr_ok,
        responses_scored=total,
        cell_geometry_agreement=round(ink_ok / ink_n, 3) if ink_n else None,
        status_agreement=round(agree / total, 3) if total else None,
        planted_misconceptions=diag_total,
        misconception_recovered=round(diag_hits / diag_total, 3) if diag_total else None,
        results=results,
    )
